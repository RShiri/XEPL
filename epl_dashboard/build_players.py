...
from xg_model import (ascii_name, SHOT_TYPES, shot_xg, match_xg_map, match_xgot_by_event,
                      is_shootout, player_xa_from_events)

MATCH_DIR = os.environ.get("EPL_MATCH_DIR") or os.path.join(ROOT, "epl", "matches")
...
def _fotmob_xgot_buckets(match_data):
    ...
    (_fotmob_shots — see epl/scraper.py's _fotmob_shot_xg_list), for the goalkeeper
    goals-prevented stat below. Same best-effort team+surname+minute matching as
    build_match_details.py's cross-provider xG buckets (XLALIGA)."""
    out = {}
...
def _keeper_shot_extras(match_data, ex, xgot_by_event):
    ...
    partial coverage) only for shots our model has no value for (penalties,
    shots missing those qualifiers). Own goals are excluded entirely (not a shot
    faced by the opposing keeper at all)."""
...
def _is_progressive(x, y, ex, ey):
    ...
    perspective (goal fixed at (100, 50) for both sides) -- no home/away flip needed."""
...
# XLALIGA's xg_core/xt_prototype.py against a full season before shipping here;
# see that file for the full method writeup (Karun Singh's formulation: a 16x12
# zone grid, empirical shot/goal/move probabilities + zone transition matrix
# from the event stream, value surface via fixed-point iteration) and the
# finding that drove the design below -- a raw per-action sum is dominated by
# deep buildup players (centre-backs, keepers), not creators. Fixed by
# crediting only StatsBomb-progressive actions (same definition as
# prog_passes/prog_carries above) and reporting per-90, not season totals.
#
# Unlike xG/xGOT, xT has no pre-trained artifact: the value surface is fit
# fresh from each SEASON's own event data, not scored from a model trained
# elsewhere -- so it's computed once per aggregate() call across every match in
# that season, then applied to score the same season's progressive actions.
...
def aggregate(match_dir=MATCH_DIR):
    ...
    for rec in players.values():
        ...
        # excluded from both sides). Positive = shot-stopping above what the shots
        # faced were worth. None for outfielders and keepers with no matched shot.
