...
(function () {
  ...
  var PORT = 8779;
  var TOKEN_KEY = "epl_control_token";
  var PUSH_KEY = "epl_control_push";        // remembered "publish when done" preference
  ...
  function run(actionOverride, overrides) {
    ...
    if (health.auth && !token()) {
      var t = prompt("Control token (EPL_CONTROL_TOKEN on the server):", "");
