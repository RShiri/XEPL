...
<!-- progress:platform -->
- **2026-09-15** — Reconciled two independent parallel sessions' work into one branch (claude/xepl-reconcile-parity), off origin/main
  Two sessions both brought XEPL to feature parity with XLALIGA on 2026-09-14 without knowing about each other. origin/main (merged PR) had the polished purple-accent design, the data/ split-bundle architecture, a new 23-feature xg_core_v3 model, and better engineering in build_data.py (WhoScored-authoritative score fix for FotMob's 0-0 glitch) and build_schedule.py (majority-vote team-name canonicalisation) -- but its underlying epl/matches/ raw scrape was stale/incomplete (2025-26 only 149/380, and ALL seasons showed 0 xG once checked, apparently because the v3 model was integrated but never run against a complete raw corpus). The other local branch (claude/scrape-24-25-season-xepl, same commit as local main, never pushed) had the complete correct raw data: all 4 historical seasons 380/380 WhoScored-scraped with events, 2026-27 at 39/380 matching real-world state. Reconciliation: branched off origin/main, kept essentially all of its code as-is after verifying function-by-function it was equal-or-better than the other branch's (v3 xG model, score-authority fix, name canonicalisation all confirmed correct/superior), copied over the complete raw epl/matches/ corpus (untracked, 3.5GB) and the freshly-verified SCHEDULE_<season>.json spines from the other branch, restored one real regression (build_split.py had dropped the styles.css/app.js/match.css/match.js/match_export.js/winprob_params.js HTML cache-busting rewrite present on the other branch -- re-added it, extended to the 2 new asset files origin/main had added), then ran the full builder chain (build_match_details -> build_players -> build_shots -> build_player_lab -> build_data -> build_database -> build_split) against the combined correct data. Confirmed the other branch's hub.html/hub.css/'Switch to Premium Hub' leftover button never existed on origin/main -- nothing to remove, nothing reintroduced. Result verified live in browser: all 4 historical seasons 380/380 with xG, 2026-27 39/380 with xG, zero console errors, zero failed network requests, Match Centre opens cleanly for a 2025-26 and a 2026-27 match.
