"""
Git operations for pushing rendered PNGs to the XLALIGA site repository (EPL section).
...
XWCTWIT_REPO   = os.environ.get("XWORLDCUPTWIT_REPO",   "https://github.com/RShiri/XLALIGA.git")
XWCTWIT_BRANCH = os.environ.get("XWORLDCUPTWIT_BRANCH", "main")
# EPL-specific so it doesn't collide with the La Liga pipeline's LALIGA_PNG_SUBDIR in a
# shared .env. Note "epl_png" (lowercase) for parity with "laliga_png".
XWCTWIT_SUBDIR = os.environ.get("EPL_PNG_SUBDIR", "epl_png")  # PNG subfolder in the repo

PROJECT_ROOT = Path(__file__).resolve().parent.parent  # repo root (holds epl_dashboard/, epl_png/, epl/)
...
def push_png_to_xworldcuptwit(png_path: str, commit_message: str | None = None) -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="xwctwit_") as tmpdir:
        ...
        # Place PNG inside the epl_png/ subfolder
        ...
        _run(["git", "config", "user.email", "eplbot@github.com"], cwd=tmpdir)
        _run(["git", "config", "user.name",  "EPL Analytics Bot"],  cwd=tmpdir)
...
def push_match_update(png_path: str, match_id: str | None = None,
    ...
    *plus* the regenerated web-dashboard outputs (data.js, players.js, the match
    ...
    repo_url = _authed_url(XWCTWIT_REPO)
    msg = commit_message or f"[EPL] {match_id} analytics dashboard"
    ...
    with tempfile.TemporaryDirectory(prefix="xwctwit_") as tmpdir:
        ...
        _copy_file_into(tmpdir, "epl_dashboard/data.js")
        _copy_file_into(tmpdir, "epl_dashboard/players.js")
        # shots.js and player_lab/ are regenerated by the render hook too — without these
        # two lines the live Team Lab and Player Lab stay frozen while everything else moves.
        _copy_file_into(tmpdir, "epl_dashboard/shots.js")
        _copy_tree_into(tmpdir, "epl_dashboard/player_lab")
        _copy_tree_into(tmpdir, "epl_dashboard/matches_detail")
        _copy_tree_into(tmpdir, "epl_dashboard/database")
        # data/ is what the site actually loads (build_split.py's per-season bundles) —
        # without this the live site keeps serving the previous build's data/*.js.
        _copy_tree_into(tmpdir, "epl_dashboard/data")

        # 3. the raw scraped match JSON (lives under epl/matches/<season>/<id>.json)
        for rel in glob.glob(os.path.join(str(PROJECT_ROOT), "epl", "matches", "*",
                                          match_id + ".json")) or []:
            _copy_file_into(tmpdir, os.path.relpath(rel, PROJECT_ROOT))
...
def _raw_url(filename: str) -> str:
    """Construct the raw GitHub URL for a file in the site repo's epl_png/ folder."""
    base = (
        XWCTWIT_REPO
        .rstrip("/")
