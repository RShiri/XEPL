...
def _describe(body: str) -> str:
    ...
    if head == "<":
        ...
        return (f"XML <{root.tag}>, {len(leagues)} league(s), {sum(l[2] for l in leagues)} match(es)"
                + (f" — PREMIER LEAGUE FOUND: {hit}" if hit else " — no the Premier League"))
    try:
        data = json.loads(body)
    except Exception:
    ...
    if isinstance(data, dict) and "leagues" in data:
        ...
        return f"JSON, {len(lg)} league(s)" + (f" — PREMIER LEAGUE FOUND: {hit}" if hit else " — no the Premier League")
...
def build_schedule(season: str, start: str | None = None, end: str | None = None,
    ...
    if not by_id:
        ...
        if verbose:
            print(f"Sweeping {len(days)} days ({s} → {e}) for Premier League "
...
def _check_team_assets(matches: "list[dict]") -> None:
    ...
    try:
        ...
        from epl.team_colors import EPL_TEAM_COLORS
        no_colour = [t for t in teams if t not in EPL_TEAM_COLORS]
    except Exception as exc:
