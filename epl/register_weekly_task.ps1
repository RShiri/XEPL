<#
.SYNOPSIS
    Registers ONE recurring Windows Scheduled Task that runs the weekly Premier League
    update (epl/weekly_update.py): refresh fixtures/results, scrape whatever
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File epl\register_weekly_task.ps1
    powershell -ExecutionPolicy Bypass -File epl\register_weekly_task.ps1 -DayOfWeek Monday -Time 09:00
#>
...
$pyArgs = "epl\weekly_update.py"
if ($NoPush) { $pyArgs += " --no-push" }

$startTime = [datetime]::ParseExact($Time, "HH:mm", $null)

Write-Host ""
Write-Host "Premier League weekly update task" -ForegroundColor Cyan
Write-Host ("=" * 70)
...
Register-ScheduledTask -TaskName $TaskName -TaskPath $TaskFolder -Action $action -Trigger $trigger `
    -Settings $settings -Description "Weekly Premier League fixtures/results refresh + scrape + push" -Force | Out-Null

Write-Host "[OK] Registered '$TaskName' - runs every $DayOfWeek at $Time." -ForegroundColor Green
Write-Host ""
Write-Host "View:    Get-ScheduledTask -TaskPath '\EPL\' -TaskName '$TaskName'"
Write-Host "Run now: Start-ScheduledTask -TaskPath '\EPL\' -TaskName '$TaskName'"
Write-Host ('Remove:  Unregister-ScheduledTask -TaskPath ''\EPL\'' -TaskName ''' + $TaskName + ''' -Confirm:$false')
Write-Host ""
