...
class XGScorer:
    ...
    def iter_match_xg(self, match_data, league=None):
        """Yield (event_id, xg) for every real shot in a WhoScored match blob, linking
        each shot to its assisting pass so the v3 extras are populated.

        WARNING: WhoScored event_id is NOT unique within a match (two shots can share
        one), so do NOT build a dict keyed by it — colliding shots would overwrite each
        other's xG. Build pipelines should use match_xg_by_id() below, which keys the map
        by the event object's identity instead."""
