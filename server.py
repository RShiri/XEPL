...
"""Local dashboard server: static site + the Scraper control API.
    ...
    py server.py                 → http://localhost:8778/epl_dashboard/index.html
...
``EPL_CONTROL_HOST`` says otherwise, and honours an optional shared secret in
``EPL_CONTROL_TOKEN`` (sent by the UI as ``X-Control-Token``).

Every job finishes by appending a row to PROGRESS.md via ``epl.progress_log``.
"""
...
LOG_DIR = ROOT / "epl" / "logs"
SCHED_DIR = ROOT / "epl" / "schedules"
MATCH_DIR = Path(os.environ.get("EPL_MATCH_DIR") or (ROOT / "epl" / "matches"))

PORT = int(os.environ.get("EPL_PORT", "8779"))
HOST = os.environ.get("EPL_CONTROL_HOST", "127.0.0.1")
TOKEN = os.environ.get("EPL_CONTROL_TOKEN", "").strip()
...
REBUILD_STEPS = [
    [PY, "epl_dashboard/build_match_details.py"],
    [PY, "epl_dashboard/build_players.py"],
    [PY, "epl_dashboard/build_shots.py"],
    [PY, "epl_dashboard/build_player_lab.py"],
    [PY, "epl_dashboard/build_database.py"],
...
def build_job(body: dict) -> Job:
    ...
    if action == "schedule":
        steps = [[PY, "epl/build_schedule.py", "--season", season],
                 [PY, "epl_dashboard/build_data.py"]]
        ...
        steps = [[PY, "epl/build_schedule.py", "--season", season]]
        optional = {0}
        cmd = [PY, "epl/backfill.py", "--season", season]
        if limit:
        ...
        steps = [[PY, f"epl/backfill.py", "--season", season, "--redo-partial"]]
        ...
        steps = [[PY, "epl/scrape_whoscored.py", "--season", season, "--ids", ids]] + REBUILD_STEPS
        ...
        steps = [[PY, "-m", "epl.run_match", "--fotmob-id", raw,
        ...
        msg = f"[EPL] dashboard refresh ({season})"
        steps = [["git", "add", "-A"],
                 ["git", "commit", "-m", msg],
                 ["git", "push"]]
    ...
    if push and action != "deploy":
        steps = steps + [["git", "add", "-A"],
                         ["git", "commit", "-m", f"[EPL] {target} ({season})"],
                         ["git", "push"]]
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def _origin_ok(self) -> str:
        ...
        if host in ("localhost", "127.0.0.1", "[::1]") or origin in SITE_ORIGINS:
            return origin
        extra = [o.strip() for o in os.environ.get("EPL_ALLOWED_ORIGINS", "").split(",") if o.strip()]
    ...
    def _api(self, method: str) -> None:
        ...
        if route == "/api/health":              # unauthenticated: just "a server is here"
            return self._json({"ok": True, "project": "XEPL", "auth": bool(TOKEN),
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Serve the Premier League dashboard (+ Scraper control API).")
    ...
    with Server((args.host, args.port), Handler) as httpd:
        print(f"Serving Premier League Dashboard on http://localhost:{args.port}/epl_dashboard/index.html")
        if Handler.control:
